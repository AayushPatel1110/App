...
import { createMainCategory, getAllActiveCategories } from "@/services/category.service"
...
export default function BusinessRegisterPage() {
  ...
  const handleEmailVerify = async () => {
    ...
    try {
      ...
      router.push(`/auth/email-otp?email=${encodeURIComponent(email)}&purpose=${PURPOSE_BUSINESS_EMAIL_VERIFY}&redirect=%2Fauth%2Fbusiness-register`)
    } catch (err) {
  ...
  const handleMobileVerify = async () => {
    ...
    if (!mobile || mobileLoading || mobileVerified || mobileCooldown > 0) return
  ...
  const handleSubmit = async () => {
    ...
    if (!emailVerified) {
      setSubmitError("Please verify business email before registration")
      return
    }
    if (!mobileVerified) {
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSubmit() }} className="business-form business-form--pro">
          ...
          <section className="business-section-card">
            ...
            <div className="business-grid business-grid--2">
              <Field label="Primary Number *">
                <div className="business-inline-action">
                  ...
                  <button
                    ...
                    disabled={!form.primaryNumber.trim() || mobileVerified || mobileLoading || mobileCooldown > 0}
                  ...
                  >
                    {mobileLoading ? "Sending..." : mobileVerified ? "Verified" : mobileCooldown > 0 ? `Retry ${mobileCooldown}s` : "Verify"}
                  </button>
                </div>
              </Field>
              ...
              <Field label="Business Email *" hint="Email verification is required before submission.">
                <div className="business-inline-action">
                  <input
                    type="email"
