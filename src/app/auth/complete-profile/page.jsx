...
import { getProducts, getDefaultProductKey, setDefaultProductKey } from "@/services/pro.service"
...
export default function CompleteProfilePage() {
  ...
  useEffect(() => {
    ...
    if (!verified) verified = sessionStorage.getItem("mobile_verified")
    if (!mobile) mobile = sessionStorage.getItem("otp_mobile")
    if (!cc) cc = sessionStorage.getItem("otp_cc")
...
function EmailField({ value, onChange, onVerify, verified, loading, cooldown }) {
  const buttonLabel = verified ? "Verified" : cooldown > 0 ? `Verify in ${cooldown}s` : "Verify"
    ...
    <div className="space-y-1.5">
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          type="button"
          onClick={onVerify}
          disabled={verified || loading || cooldown > 0 || !emailRegex.test(value.trim())}
      ...
      <p className={`text-xs ${verified ? "text-emerald-600" : "text-slate-500"}`}>
        {verified ? "Email verified successfully." : "Verify your email to continue."}
      </p>
    </div>
  )
}
